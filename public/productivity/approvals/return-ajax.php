<?php
/**
 * إرجاع سجل إنتاجية يومي للتعديل - AJAX
 * Return Daily Productivity Log for Revision - AJAX
 */
...
// التحقق من الصلاحيات
if (!hasPermission('productivity_approvals_return')) {
    http_response_code(403);
    echo json_encode([
        'success' => false,
        'message' => 'ليس لديك صلاحية لإرجاع السجلات للتعديل'
    ]);
    exit();
}
...
try {
    ...
    if (empty(trim($comments))) {
        throw new Exception('ملاحظات الإرجاع مطلوبة');
    }
    ...
    if ($log['status'] !== 'submitted') {
        throw new Exception('لا يمكن إرجاع هذا السجل. الحالة الحالية: ' . $log['status']);
    }
    ...
    if (!hasPermission('productivity_daily_logs_view_all_branches') && 
        ...
        throw new Exception('ليس لديك صلاحية لإرجاع سجلات هذا الفرع');
    }
    
    // إرجاع السجل للتعديل
    $result = $approvalModel->returnForRevision($logId, $_SESSION['user_id'], $comments);
    
    if ($result) {
        echo json_encode([
            'success' => true,
            'message' => 'تم إرجاع السجل للتعديل',
            'log_id' => $logId
        ]);
    } else {
        throw new Exception('فشل في إرجاع السجل للتعديل');
    }
