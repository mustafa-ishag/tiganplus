    ...
    try {
        ...
        if (isset($_POST['works']) && is_array($_POST['works'])) {
            foreach ($_POST['works'] as $workData) {
                if (!empty($workData['work_item_id']) && isset($workData['quantity'])) {
                    $works[] = [
        ...
        if ($existingCert) {
            ...
            if (!empty($works)) {
                ...
                foreach ($works as $work) {
                    // جلب بيانات العمل من جدول العقد
                    ...
                    if ($workInfo) {
                        $unitPrice = (float)$workInfo['price'];
...
<!-- نموذج إنشاء الشهادة -->
<form method="POST" id="createCertificateForm" hx-boost="false">
    <!-- المعلومات الأساسية -->
    <div class="card mb-4">
...
var workItemsData = []; // سيتم تعبئتها عبر AJAX
    ...
    fetch(`../../contracts/get-work-items-ajax.php?work_order_id=${workOrderId}`)
        ...
        .then(res => {
            if (res.success) {
                ...
                // تحديث اقتراحات الأعمال في جميع الصفوف المفتوحة
                document.querySelectorAll('.work-search-input').forEach(input => {
                    input.value = ''; // مسح القيم القديمة لأنها لم تعد صالحة
                });
                document.querySelectorAll('select[name*="work_item_id"]').forEach(select => {
                    select.innerHTML = '<option value="">اختر البند</option>'; // مسح الخيارات القديمة
                });
            } else {
            ...
            <div class="material-select-container position-relative">
                ...
                <select name="materials[${materialRowCounter}][material_id]" class="form-select form-select-sm d-none" onchange="updateMaterialInfo(this, ${materialRowCounter})" required>
                    ...
                    ${materialsData.map(material =>
                        `<option value="${material.id}" data-code="${String(material.item_number || '').replace(/"/g, '&quot;')}" data-description="${String(material.description || '').replace(/"/g, '&quot;')}" data-unit="${String(material.unit || '').replace(/"/g, '&quot;')}" data-group="${String(material.group_number || '').replace(/"/g, '&quot;')}">${material.item_number}</option>`
                    ).join('')}
                </select>
            ...
            <div class="work-select-container position-relative">
                ...
                <select name="works[${workRowCounter}][work_item_id]" class="form-select form-select-sm d-none" onchange="updateWorkInfo(this, ${workRowCounter})" required>
                    ...
                    ${workItemsData.map(workItem =>
                        `<option value="${workItem.id}" data-code="${String(workItem.item_number || '').replace(/"/g, '&quot;')}" data-description="${String(workItem.description || '').replace(/"/g, '&quot;')}" data-unit="${String(workItem.unit || '').replace(/"/g, '&quot;')}" data-price="${workItem.price || 0}">${workItem.item_number}</option>`
                    ).join('')}
                </select>
...
// تم دمج التحديث في دالة التهيئة الشاملة
</script>
