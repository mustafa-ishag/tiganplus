<?php
/**
 * صفحة تحديث رقم صحيفة الإدخال للمستخلصات النهائية العادية
 * Update SAP Entry Sheet Number for Final Regular Extracts
 */
...
$pageTitle = 'تحديث رقم صحيفة الإدخال (SAP) - المستخلصات النهائية العادية';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['sap_file'])) {
        ...
        for ($i = 1; $i < count($data); $i++) {
            ...
            $paymentDate = isset($row[$paymentDateCol]) && $row[$paymentDateCol] !== null
                ...
                // التحقق من أن المستخلص نهائي للجزئية (مبلغ الضريبة > 0)
                if ($taxAmount <= 0) {
                    $skippedCount++;
                    continue; // تخطي المستخلصات الجزئية (الضريبة = 0)
                }

                // التحقق من أن رقم المستخلص يبدأ بحرف F
                if (!preg_match('/^F/i', $extractNumber)) {
                    $skippedCount++;
                    continue;
                }

                // إزالة حرف F من رقم المستخلص للبحث في قاعدة البيانات
                // في قاعدة البيانات، أرقام المستخلصات مخزنة بدون حرف F
                ...
                // البحث عن المستخلص في قاعدة البيانات (بدون حرف F)
                ...
                $checkDuplicate = $db->prepare("SELECT extract_number FROM final_regular_extracts WHERE entry_sheet_number = ? AND id != ?");
                ...
                $validRecords[] = [
                    ...
                    'po_number' => $poNumber,
                    'entry_sheet_number' => $entrySheetNumber,
                    'disbursed_date' => $formattedDisbursementDate,
                ...
                <div class="col-md-4">
                    <div class="card border-left-info">
                        <div class="card-body">
                            ...
                            <ul class="small mb-3">
                                <li>مبلغ الضريبة > 0 (مستخلص نهائي)</li>
                                <li>رقم المستخلص يبدأ بحرف F</li>
                            ...
                            <div class="alert alert-warning py-2 px-2 mb-0" style="font-size: 0.75rem;">
                                ...
                                حرف <strong>F</strong> في ملف SAP للدلالة فقط على أن المستخلص نهائي.<br>
                                عند البحث في قاعدة البيانات، يتم <strong>إزالة حرف F</strong> تلقائياً.<br>
                                <small class="text-muted">مثال: F2501000001 → يبحث عن 2501000001</small>
                            </div>
                        </div>
                    </div>
                </div>
