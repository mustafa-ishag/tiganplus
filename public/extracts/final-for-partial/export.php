<?php
/**
 * صفحة تصدير المستخلصات النهائية للجزئية
 * Final For Partial Extracts Export Page
 */
...
$pageTitle = 'تصدير المستخلصات النهائية للجزئية';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['export'])) {
        require_once __DIR__ . '/../../../includes/FinalForPartialExtractExcelExporter.php';
        ...
        $exporter = new FinalForPartialExtractExcelExporter($db, $user_id, $filters);
        $exporter->export();
        exit();
    }
    ...
    $statsQuery = "
        SELECT 
            ...
            COUNT(DISTINCT ffpewo.work_order_id) as total_work_orders,
            SUM(ffpe.total_amount) as total_amount,
            SUM(ffpe.tax_amount) as total_tax,
            SUM(ffpe.total_penalty_amount) as total_penalty,
            SUM(ffpe.net_amount) as total_net
    ...
    $recentLogsQuery = "
        ...
        WHERE ffpel.operation_type = 'export'
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                <?php echo number_format($stats['total_penalty'] ?? 0, 2); ?> ر.س
                            </div>
                        </div>
                        <div class="col-auto">
