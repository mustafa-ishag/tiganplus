<?php
/**
 * صفحة تحديث رقم صحيفة الإدخال للمستخلصات النهائية للجزئية
 * Update SAP Entry Sheet Number for Final For Partial Extracts
 */
...
$pageTitle = 'تحديث رقم صحيفة الإدخال (SAP) - المستخلصات النهائية للجزئية';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['sap_file'])) {
        ...
        for ($i = 1; $i < count($data); $i++) {
            ...
            $paymentDate = isset($row[$paymentDateCol]) && $row[$paymentDateCol] !== null
                ...
                $checkDuplicate = $db->prepare("SELECT extract_number FROM final_for_partial_extracts WHERE entry_sheet_number = ? AND id != ?");
